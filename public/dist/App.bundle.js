...
function ajaxHeart(e) {
  ...
  fetch(this.action, {
    ...
    if (isHearted) _this.heart.classList.toggle('heart__button--float');

    (0, _bling.$)('.heart-count').textContent = res.favorites.length;
  }).catch(console.error);
}
...
function loadPlaces(map, lat, lng) {
  fetch('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    return res.json();
  }).then(function (res) {
    if (!res.length) {
...
function makeMap(mapDiv) {
  if (!mapDiv) return;
  getCoordsAsync().then(function (_ref) {
...
var dompurify = __webpack_require__(6);
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    fetch('/api/search?q=' + this.value).then(function (res) {
      return res.json();
    }).then(function (res) {
      if (res.length) {
        searchResults.innerHTML = dompurify.sanitize(searchResultsToHTML(res));
      } else {
  ...
  searchInput.on('keyup', function (e) {
    ...
    if (e.keyCode === 40) {
      ...
      current = current && current.nextElementSibling || searchResults.firstElementChild;
      current.classList.add(activeClass);
    } else if (e.keyCode === 38) {
      current ? current.classList.remove(activeClass) : null;
      current = current && current.previousElementSibling || searchResults.lastElementChild;
      current.classList.add(activeClass);
    } else {
